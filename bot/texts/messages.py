...
def btn_confirm_cancel() -> str:
    return "Да, отменить и отвязать карту"
...
def cancel_confirm() -> str:
    return (
        ...
        "• Привязанная карта будет удалена — автосписания больше не произойдут.\n"
...
def cancel_done() -> str:
    return (
        "Подписка отменена, карта отвязана.\n\n"
        "Автосписания больше не будут производиться. "
        "Доступ к каналу сохраняется до конца оплаченного периода.\n\n"
        "Чтобы оформить подписку снова — /start."
    )
